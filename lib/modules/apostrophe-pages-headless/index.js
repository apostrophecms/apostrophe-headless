  ...
  construct: function(self, options) {
    ...
    self.addRestApiRoutes = function() {
      ...
      self.apos.app.get(endpoint, function(req, res) {
        ...
        var result;
        return async.series([ findPages ], function(err) {
          if (err) {
            console.error(err);
            return res.status(500).send({ error: 'error' });
          }
      ...
      self.apos.app.get(endpoint + '/:id', function(req, res) {
        ...
        return self.findForRestApi(req).and({ _id: id }).toObject(function(err, page) {
          if (err) {
            return res.status(500).send({ 'error': 'error' });
          }
          if (!page) {
      ...
      self.apos.app.post(endpoint + '/:id/move', function(req, res) {
        ...
        var position = self.apos.launder.id(req.body.position);
