      ...
      self.apos.app.get(endpoint, function(req, res) {
        ...
        function findPieces(callback) {
          return cursor.toArray(function(err, pieces) {
            ...
            // Hook
            if (self.apos.modules[self.name] && self.apos.modules[self.name].afterFindMany) {
              pieces = self.apos.modules[self.name].afterFindMany(req, pieces);
            }
      ...
      self.apos.app.get(endpoint + '/:id', function(req, res) {
        ...
        function find(callback) {
          return self.findForRestApi(req).and({ _id: id }).toObject(function(err, _piece) {
            ...
            piece = _piece;
            // Hook
            if (self.apos.modules[self.name] && self.apos.modules[self.name].afterFindOne) {
              piece = self.apos.modules[self.name].afterFindOne(req, piece);
            }
            // Attach `_url` and `_urls` properties
